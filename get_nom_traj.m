...
function [xLd,vLd,aLd,l,dl,d2l,p,dp,d2p,R,Omega,dOmega,daLd,d2aLd,d3p,dR,d2R,f,M] = get_nom_traj(params, load_traj)
    mQ = params.mQ ;
    mL = params.mL ;
    J = params.J ;
    g = params.g ;
...
%     dp = (dTp*norm_Tp - p*norm_Tp*dnorm_Tp)/norm_Tp^2 ;
    ...
    d2Tp = -mL*d2aLd ;
%     d2norm_Tp = ( (vec_dot(dTp, dTp)+vec_dot(Tp, d2Tp))*norm_Tp - vec_dot(Tp, dTp)*dnorm_Tp ) / norm_Tp^2 ;
%     d2norm_Tp = ( (vec_dot(dTp, dTp)+vec_dot(Tp, d2Tp))*norm_Tp - dnorm_Tp*norm_Tp*dnorm_Tp ) / norm_Tp^2 ;
...
%     d2p = ( (d2Tp - dp*dnorm_Tp-p*d2norm_Tp)*norm_Tp - (dp*norm_Tp)*2*norm_Tp*dnorm_Tp ) / norm_Tp^2 ;
    ...
    d3Tp = -mL*d3aLd ;
%     d3norm_Tp = ( (2*vec_dot(dTp, d2Tp) + vec_dot(dTp, d2Tp)+vec_dot(Tp, d3Tp) - 2*dnorm_Tp*d2norm_Tp )*norm_Tp - d2norm_Tp*norm_Tp*dnorm_Tp  ) ...
%         / norm_Tp^2 ;
...
%     - d2p*norm_Tp*dnorm_Tp) / norm_Tp^2 ;
    ...
    vxQ = vLd - l*dp - dl*p;
    axQ = aLd - l*d2p - 2*dl*dp - d2l*p;
    daxQ = daLd - l*d3p - dl*d2p - 2*d2l*dp - 2*dl*d2p - d3l*p - d2l*dp;
    d2axQ = d2aLd - l*d4p - dl*d3p - d2l*d2p - dl*d3p - 2*d3l*dp - 2*d2l*d2p...
        - 2*d2l*d2p - 2*dl*d3p - d4l*p - d3l*dp - d3l*p -d2l*dp;
